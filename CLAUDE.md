...
| `train_20260325_12xxxx` | `20260325_18xxxx` | **0.3m** | 新容差，第一轮推理（Dr Sun 不信任此轮结果） |
| `train_20260325_12xxxx` | `20260326_004xxx` | **0.3m** | 新容差，第二轮推理（`scripts/run_all_infer_only.sh`，多并行验证） |

- `.1` 后缀目录为旧版（goal_tolerance=1.0m 时期）的推理结果
- `20260325_18xxxx` 为 0.3m 第一轮推理，Dr Sun 认为不可信，做了第二轮 `20260326_004xxx` 验证
- 两轮推理使用相同的 0.3m 重训模型（`train_20260325_12xxxx`），仅推理框架不同
...
- **论文润色工作流（强制）**：修改论文段落时执行以下流程：
  1. 多 Agent 并行搜索同领域真实论文中**同类段落**的原文句子（非摘要，要完整句子）
  2. 提炼这些句子的**句式特征**（如：零铺垫直接说事、参数嵌入描述、一句话完成结构说明），而非逐句翻译成中文
  3. 按提炼出的句式风格改写，每句话标注对标的论文原句
  4. 自检：每句话删掉是否丢失信息？不丢失就该删。禁止"为兼顾X与Y""为了实现X"等空洞动机句，禁止术语堆砌压缩
