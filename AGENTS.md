...
3) 默认中文回复；论文正文先中文写作，定稿后统一英文润色。README 等项目文档也用中文撰写。论文中尽可能少用括号，改用"如图…所示""即…"等行文方式替代。公式中使用专业数学符号，禁止出现代码风格变量名如 `grid_size`、`cell_size`，应使用 $\Delta c$、$\delta$、$\epsilon$ 等标准记法。
4) **严禁凭记忆生成 BibTeX**——未核实引用标 `[CITATION NEEDED]`。
5) `AGENTS.md` 为唯一项目规则文件。
...
15) **学术问题必须先读论文**：回答任何与本项目相关的学术问题前，必须先读 `paper/main.tex` 及相关章节，基于论文实际内容回答，严禁凭印象或通用知识敷衍作答。
16) **复杂任务默认多 Agent 集群**：涉及多文件修改、跨模块调研、论文+代码联动等复杂任务时，默认启用多 Agent 并行（`Agent` 工具），将独立子任务分派给专用 subagent 并发执行，最大化效率。简单单文件任务无需启用。
...
- **3-algo filter**：CNN-DDQN+MD + RRT* + LO-HA*。
...
- **核心算法**：CNN-DDQN+MD，reward_k_t=0.2（消融实验 2026-03-15 确认 MD 全面优于 Duel）。
- **若结果不支持叙事**：换 checkpoint 或调训练参数重跑，不可篡改数据。

### 3.3 核心对比实验：CNN-DDQN+MD vs RRT* vs LO-HA*（全局规划，无 MPC）
...
- **变体**: CNN-DDQN, CNN-DDQN+Duel, CNN-DDQN+MHA, CNN-DDQN+MD, CNN-DQN, CNN-DQN+Duel, CNN-DQN+MHA, CNN-DQN+MD, MLP-DQN, MLP-DDQN
- **结论**: CNN-DDQN+MD Quality 路径长度 Long/Short 均第一（26.364m / 8.603m），SR 稳定（94%/92%）
