#!/usr/bin/env python3
"""Batch launcher: Table2 all-MPC rerun, 10 seeds x 2 suites in parallel."""
...
TEMPLATES = {
    "long": PROJ / "configs" / "repro_20260324_allmpc_sr_long.json",
    "short": PROJ / "configs" / "repro_20260324_allmpc_sr_short.json",
}

CFG_DIR = PROJ / "configs" / "repro_allmpc"
...
def make_config(suite: str, seed: int) -> pathlib.Path:
    with open(TEMPLATES[suite]) as f:
        cfg = json.load(f)
    cfg["infer"]["seed"] = seed
    cfg["infer"]["out"] = f"repro_20260324_allmpc_sr_{suite}"
    out = CFG_DIR / f"allmpc_s{seed}_{suite}.json"
